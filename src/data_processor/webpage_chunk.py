        ...
        if code:
            ...
            docs.append(Document(
                ...
                metadata={
                    ...
                    "has_code": True, "is_code_block": True,
                    "code_description": desc, "domain": "software_engineering",
                },
            ))
        pre.decompose()
    return docs
    ...
    for i, doc in enumerate(docs):
        ...
        if doc.metadata.get("is_code_block"):
            ...
            print(f"  [{i+1}/{total}] [{doc.metadata.get('section_heading', '')}] "
                  f"{m.get('content_type')} | {m.get('domain')}")
    return docs
...
def run(urls: list[str] = URLS) -> list[Document]:
    ...
    store(all_docs)
    print(f"\n✅  Stored {len(all_docs)} chunks from {len(urls)} pages")
    return all_docs
